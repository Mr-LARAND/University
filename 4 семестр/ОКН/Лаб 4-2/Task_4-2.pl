% patient_card(НомерКарты, ФИО, Адрес, Телефон, Возраст, МестоРаботы)
...
% Найти пациентов, которых принимал заданный врач.
